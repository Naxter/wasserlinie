...
LEVEL_COLUMNS = ["station", "ts", "value"]
...
def merge_levels(existing: pd.DataFrame | None, fresh: pd.DataFrame, now: datetime) -> pd.DataFrame:
    ...
    frames = [f for f in (existing, fresh) if f is not None and len(f)]
    if not frames:
        return pd.DataFrame(columns=LEVEL_COLUMNS)
...
def run(paths: Paths, days: int = 31, workers: int = 6) -> None:
    ...
    payload = {"generated": now.isoformat(timespec="seconds"), "stations": station_records(stations, levels)}
    write_json(paths.stations, payload)
    log.info("stations.json: %d stations", len(stations))
